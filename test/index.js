...
describe('create()', function () {
    ...
    it('should use captureStackTrace if available', function (done) {

        var captureStackTrace = Error.captureStackTrace;
        var captureStackTraceCalled = false;
        ...
        Boom.create(500, 'An error');

        expect(captureStackTraceCalled).to.equal(true);

        captureStackTraceCalled = false;
        Error.captureStackTrace = undefined;

        Boom.create(500, 'An error');

        expect(captureStackTraceCalled).to.equal(false);
